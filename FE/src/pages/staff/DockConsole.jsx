...
export default function DockConsole() {
  ...
  async function onDock() {
    ...
    try {
      ...
      toast(
        data?.message ||
          `Đã gửi yêu cầu dock: ${batteryIdIn} → slot ${slotIdIn}`
      );
    } catch (e) {
      console.error(e);
  ...
  return (
    <section>
      ...
      <p className="muted">
        Gửi đúng 4 tham số: <b>slotId, batteryId, stationId, staffId</b> (GET).
      </p>
      ...
      <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", gap: 24 }}>
          ...
          >
            <label>
              ...
              <input
                className="input"
                value={slotIdIn}
                onChange={(e) => setSlotIdIn(e.target.value)}
              />
            </label>
            <label>
              Battery Id
              <input
                className="input"
                value={batteryIdIn}
                onChange={(e) => setBatteryIdIn(e.target.value)}
              />
            </label>
          </div>
        ...
        <div className="card">
          ...
          <div className="small muted">
            Không load slots khi vào trang. Chỉ gửi request dock sang BE bằng
            GET.
            <br />
            Nếu BE cần tên key khác (ví dụ{" "}
            <code>SlotId/BatteryInId/StataionId/staffId</code>), hàm API đã tự
            thử các biến thể đó. Nếu vẫn lỗi, lỗi BE sẽ hiện ra nguyên văn.
          </div>
        </div>
      </div>
    </section>
