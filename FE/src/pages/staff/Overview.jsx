...
export default function Overview() {
  ...
  const fetchOverview = async () => {
    try {
      ...
      setErr(
        e?.response?.data?.message || e?.message || "Tải overview thất bại"
      );
      setData(null);
    } finally {
      setLoading(false);
    ...
    return [
      {
        ...
        icon: "🔋",
        label: "Fully Charged",
        value: n.numberOfBatteryFully,
      },
      {
        key: "charging",
        icon: "🔌",
        label: "Charging",
        value: n.numberOfBatteryCharging,
      },
      {
        key: "maintenance",
        icon: "🛠️",
        label: "Maintenance",
        value: n.numberOfBatteryMaintenance,
      },
      {
        key: "warehouse",
        icon: "📦",
        label: "In Warehouse",
        value: n.numberOfBatteryInWarehouse,
      },
      {
        key: "swaps",
        icon: "⚡",
        label: "Swaps Today",
        value: data?.swapInDat,
      },
    ];
  }, [data]);
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Tổng quan trạm & hoạt động trong ngày
          </p>
        </div>
