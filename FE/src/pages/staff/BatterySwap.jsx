...
function formatTime12h(hms) {
  ...
  hh = hh % 12 || 12;
  return `${String(hh).padStart(2, "0")}:${String(parseInt(m, 10)).padStart(
    2,
    "0"
  )}${ampm}`;
}
    ...
    <section>
      ...
      <div className="row-between">
        <div>
          ...
          <p className="muted">
            Lịch sử đổi pin tại trạm (theo UserId của nhân viên).
          </p>
        </div>
      ...
      {/* Error */}
      {!!err && <div className="card card-padded mt-3 error">⚠️ {err}</div>}
            ...
            {loading ? (
              ...
              sorted.map((r, idx) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div>
                      In: <b>{valOrDash(r.batteryIn)}</b>
                    </div>
                    <div>
                      Out: <b>{valOrDash(r.batteryOut)}</b>
                    </div>
                  </td>
